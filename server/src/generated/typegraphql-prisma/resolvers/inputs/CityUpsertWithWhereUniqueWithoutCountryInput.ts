import * as TypeGraphQL from "type-graphql";
import * as GraphQLScalars from "graphql-scalars";
import { Prisma } from "@prisma/client";
import { DecimalJSScalar } from "../../scalars";
import { CityCreateWithoutCountryInput } from "../inputs/CityCreateWithoutCountryInput";
import { CityUpdateWithoutCountryInput } from "../inputs/CityUpdateWithoutCountryInput";
import { CityWhereUniqueInput } from "../inputs/CityWhereUniqueInput";

@TypeGraphQL.InputType("CityUpsertWithWhereUniqueWithoutCountryInput", {})
export class CityUpsertWithWhereUniqueWithoutCountryInput {
  @TypeGraphQL.Field(_type => CityWhereUniqueInput, {
    nullable: false
  })
  where!: CityWhereUniqueInput;

  @TypeGraphQL.Field(_type => CityUpdateWithoutCountryInput, {
    nullable: false
  })
  update!: CityUpdateWithoutCountryInput;

  @TypeGraphQL.Field(_type => CityCreateWithoutCountryInput, {
    nullable: false
  })
  create!: CityCreateWithoutCountryInput;
}
